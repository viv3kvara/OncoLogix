...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO doctor (d_name, experience, specialization, d_photo) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("siss", $doctor_name, $experience, $specialization, $doctorImage);
    $stmt->execute();
    $d_id = $stmt->insert_id;
    ...
    $stmt = $conn->prepare("INSERT INTO cases (p_id, d_id, h_id, t_id, cancer_type) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iiiis", $p_id, $d_id, $h_id, $t_id, $cancer_name);
    $stmt->execute();
        ...
        <form id="myform" action="./form.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="bg-gray-50/50 p-4 rounded-lg shadow-md backdrop-blur-md">
                    <h3 class="text-lg font-semibold mb-2 text-gray-800">Case ID</h3>
                    <input type="text" name="case_id" class="w-full p-2 border rounded-md" placeholder="Enter Case ID" required>
                </div>
                <div class="bg-gray-50/50 p-4 rounded-lg shadow-md backdrop-blur-md">
